									...
									if(isset($_GET["tmp"])){
										if($_GET["tmp"]=="Banh"){
											...
											while($row = mysqli_fetch_row($kq2)){
												...
												<td>'.$row[2].'</td>
												<td>'.$row[3].'</td>
												<td><img src="img/'.$row[4].'" width="50"></td>
												';
												$query1= "SELECT tenloai FROM loaibanh where id =".$row[5];
												$kq3 = mysqli_query($conn,$query1);
											
													while($row1 = mysqli_fetch_row($kq3)){
														echo '<td>'.$row1[0].'</td>';
												}
												echo '<td><a href=quanly.php?tmp1=Banh&edited=Banh&id='.$row[0].' style="text-decoration: none; color:gray;">Edit</a>
